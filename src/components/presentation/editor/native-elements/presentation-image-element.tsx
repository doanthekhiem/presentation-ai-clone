...
import { Resizable, ResizeHandle, mediaResizeHandleVariants } from "@/components/text-editor/plate-ui/resizable";
...
export const PresentationImageElement = withHOC(
  ResizableProvider,
  withRef<typeof PlateElement, PresentationImageElementProps>(({ children, className, nodeProps, ...props }, ref) => {
    ...
    useEffect(() => {
      ...
      if (hasHandledGenerationRef.current || !props.element.query || props.element.url || imageUrl) {
        return;
      }
    ...
    }, [props.element.query, props.element.url, imageUrl, props.element.setNodeValue]);

    return (
      <>
        <MediaPopover plugin={ImagePlugin}>
          <PlateElement ref={ref as React.Ref<HTMLDivElement>} className={cn(className)} {...props}>
            <div ref={imageRef}>
              <Resizable
                align={align}
                options={{
                ...
                {isGenerating ? (
                  ...
                  >
                    <Image
                      ...
                      src={imageUrl}
                      onError={(e) => {
                        console.error("Presentation image failed to load:", e, imageUrl);
                      }}
                      {...nodeProps}
                    />
                  </div>
